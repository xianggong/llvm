...
private:
  ...
  // Multi2sim
  bool SelectMTBUFOffsetM2S(SDValue In, SDValue &offset, SDValue &offen,
                            SDValue &glc, SDValue &idxen, SDValue &addr64,
                            SDValue &dfmt, SDValue &nfmt, SDValue &vaddr,
                            SDValue &srsrc, SDValue &slc, SDValue &tfe,
                            SDValue &soffset) const;
  bool SelectMUBUFScratchM2S(SDValue In, SDValue &srsrc, SDValue &vaddr,
...
bool AMDGPUDAGToDAGISel::SelectMUBUFAddr64(SDValue Addr, SDValue &SRsrc,
                                           ...
                                           SDValue &SLC, SDValue &TFE) const {
  // Force global load to use SelectMTBUFOffset
  if (Subtarget->isM2S())
    return false;
...
bool AMDGPUDAGToDAGISel::SelectMUBUFOffset(SDValue Addr, SDValue &SRsrc,
                                           ...
                                           SDValue &GLC, SDValue &SLC,
                                           SDValue &TFE) const {
  // Force to use SelectMUBUFScratchM2S
  if (Subtarget->isM2S())
    return false;
...
// Multi2Sim complex selection pattern
bool AMDGPUDAGToDAGISel::SelectMTBUFOffsetM2S(SDValue In, SDValue &offset,
                                              SDValue &offen, SDValue &glc,
                                              SDValue &idxen, SDValue &addr64,
                                              SDValue &dfmt, SDValue &nfmt,
                                              SDValue &vaddr, SDValue &srsrc, 
  ...
  if ((In.getOpcode() == ISD::ADD) && (In.getValueType() == MVT::i32)) {
    ...
    if (N1.getOpcode() == ISD::SHL) {
      ...
      SDValue newSHL = SDValue(
          ...
          0);
      CurDAG->ReplaceAllUsesWith(N1, newSHL);
...
bool AMDGPUDAGToDAGISel::SelectMUBUFScratchM2S(SDValue In, SDValue &srsrc,
                                               SDValue &vaddr, SDValue &soffset,
                                               SDValue &offset) const {
  if (!Subtarget->isM2S())
    return false;
  ...
  SDLoc DL = SDLoc(In);
  soffset = CurDAG->getTargetConstant(0, DL, MVT::i16);
  offset = CurDAG->getTargetConstant(0, DL, MVT::i16);

  if (In.getOpcode() == ISD::INTRINSIC_W_CHAIN)
  {
    vaddr = In.getOperand(2);
    srsrc = In.getOperand(3);
