...
void SILowerUAVPass::LowerGetUAV(MachineInstr &MI, unsigned PtrUavTableRegs) {
  ...
  // Add PtrUAVTable LiveIn and a COPY for def virtual register
  MBB.addLiveIn(AMDGPU::SGPR2_SGPR3);
  BuildMI(MBB, &MI, DL, TII->get(TargetOpcode::COPY),
          MRI->getLiveInVirtReg(PtrUavTableRegs)).addReg(AMDGPU::SGPR2_SGPR3);
...
void SILowerUAVPass::LowerPacUAV(MachineInstr &MI) { MI.eraseFromParent(); }
